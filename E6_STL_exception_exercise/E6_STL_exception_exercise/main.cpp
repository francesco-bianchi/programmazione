...
std::vector<GameCharacter*>createMonsters(int startX, int startY, const int numMonsters, const Dungeon &map) {
    std::vector<GameCharacter*> result; //il vettore solitamente si usa per modificare la coda (altrimenti si usa list o deque)
    result.reserve(numMonsters); //alloca memoria per un tot di elementi(si può ingrandire se si inserisce/elimina in coda)
    ...
    for(int i=0; i<numMonsters; i++) {
        ...
        result.push_back(enemy); //inserisce in coda
    }
    return result;
}
...
bool checkMonsterPosition(int x, int y, const std::vector<GameCharacter*>  &enemies, char &renderSymbol) {
...
int main() {
    ...
    auto enemies = createMonsters(hero->getPosX(), hero->getPosY(), NUM_MONSTERS, map);
