...
void Dungeon::saveToFile(std::string fileName) throw(GameFileException) {
    std::ofstream out;
    ...
    try {
        out.open(fileName);
        out << MAP_FILE_SIGNATURE << std::endl;
        out << xsize << "\n" << ysize << std::endl;
        for (int y = 0; y < ysize; y++) {
            for (int x = 0; x < xsize; x++) {
        ...
        std::cerr << e.what();
        throw GameFileException(e.what(), fileName, false);
    }
}

void Dungeon::loadFromFile(std::string fileName) throw(GameFileException, std::runtime_error, std::out_of_range) {
    std::ifstream in;
    // Set exceptions to be thrown on failure
    // @see http://en.cppreference.com/w/cpp/io/ios_base/iostate
    ...
    try {
        ...
        throw GameFileException(e.what(), fileName, true);
    }
    std::string fileLine;
    std::getline(in, fileLine);
    if (fileLine.compare(MAP_FILE_SIGNATURE) != 0)
        throw GameFileException("Invalid map file", fileName, true);
    std::getline(in, fileLine);
    xsize = std::stoi(fileLine);
    std::getline(in, fileLine);
    ysize = std::stoi(fileLine);
    int y = 0;
    while(!in.eof() && std::getline(in, fileLine)) {
        for (int x = 0; x < xsize; x++) {
            try {
                TileType cellType = TileStringToTileType(fileLine.at(x));
    ...
    if (y!=ysize)
        throw runtime_error("Map height size wrong. Expected "+std::to_string(ysize)+" but read: " + std::to_string(y));
    in.close();
}
